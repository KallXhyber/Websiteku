...
// --- Komponen Modal Verifikasi (Diperbarui dengan Notifikasi) ---
const VerificationModal = ({ request, onClose }) => {
  ...
  return React.createElement(motion.div, { className: 'fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-[60] p-4' },
    React.createElement(motion.div, { className: 'bg-discord-dark rounded-lg p-6 w-full max-w-2xl relative text-white' },
      ...
      React.createElement('div', { className: 'grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 text-center' },
        React.createElement('a', { href: request.discordUrl, target: '_blank', rel: 'noopener noreferrer', className: 'bg-discord-darker p-2 rounded' }, 'Lihat SS Discord'),
        React.createElement('a', { href: request.steamUrl, target: '_blank', rel: 'noopener noreferrer', className: 'bg-discord-darker p-2 rounded' }, 'Lihat SS Steam'),
        React.createElement('a', { href: request.cfxUrl, target: '_blank', rel: 'noopener noreferrer', className: 'bg-discord-darker p-2 rounded' }, 'Lihat SS Cfx.re')
      ),
...
const AccountFormModal = ({ account, onClose }) => {
    ...
    const handleSubmit = async (e) => { e.preventDefault(); setIsLoading(true); setError(''); try { let imageUrl = account?.imageUrl || ''; if (imageFile) { const filePath = `public/${Date.now()}-${imageFile.name}`; const { data, error: uploadError } = await supabase.storage.from('account-images').upload(filePath, imageFile); if (uploadError) throw uploadError; const { data: { publicUrl } } = supabase.storage.from('account-images').getPublicUrl(data.path); imageUrl = publicUrl; } const dataToSave = { ...formData, price: Number(formData.price), imageUrl, adminId: userData.adminId, adminName: userData.displayName || userData.email.split('@')[0], adminWa: userData.whatsapp || '', isSold: account?.isSold || false, updatedAt: serverTimestamp() }; if (account) { await updateDoc(doc(db, 'game_accounts', account.id), dataToSave); } else { await addDoc(collection(db, 'game_accounts'), { ...dataToSave, createdAt: serverTimestamp() }); } onClose(); } catch (err) { console.error("Error saving account:", err); setError("Gagal menyimpan data."); } finally { setIsLoading(false); } };
    return React.createElement(motion.div, { className: 'fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-[60] p-4' },
        React.createElement(motion.div, { className: 'bg-discord-dark rounded-lg p-6 w-full max-w-lg relative text-white' },
            ...
            React.createElement('form', { onSubmit: handleSubmit, className: 'space-y-4' },
                ...
                React.createElement('div', { className: 'flex justify-end gap-4 mt-4' }, React.createElement('button', { type: 'button', onClick: onClose, className: 'bg-gray-600 py-2 px-4 rounded' }, 'Batal'), React.createElement('button', { type: 'submit', disabled: isLoading, className: 'bg-discord-blurple py-2 px-4 rounded' }, isLoading ? 'Menyimpan...' : 'Simpan'))
            )
        )
    );
};

// --- Modal Input Nama Pengguna & Durasi ---
const SetPCUsedModal = ({ onSubmit, onClose }) => {
    ...
    return React.createElement(motion.div, { className: 'fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-[60] p-4' },
        React.createElement(motion.div, { className: 'bg-discord-dark rounded-lg p-6 w-full max-w-sm text-white' },
            ...
            React.createElement('div', { className: 'space-y-4' }, React.createElement('div', null, React.createElement('label', { className: 'text-sm text-discord-gray' }, 'Nama Pengguna'), React.createElement('input', { type: 'text', value: name, onChange: (e) => setName(e.target.value), className: 'w-full bg-discord-darker p-2 rounded mt-1', placeholder: 'Nama...' })), React.createElement('div', null, React.createElement('label', { className: 'text-sm text-discord-gray' }, 'Durasi Sewa (Jam)'), React.createElement('input', { type: 'number', value: duration, onChange: (e) => setDuration(Number(e.target.value)), className: 'w-full bg-discord-darker p-2 rounded mt-1', min: '1' }))),
            React.createElement('div', { className: 'flex justify-end gap-4 mt-6' }, React.createElement('button', { onClick: onClose, className: 'bg-gray-600 py-2 px-4 rounded' }, 'Batal'), React.createElement('button', { onClick: () => onSubmit(name, duration), className: 'bg-discord-blurple py-2 px-4 rounded' }, 'Simpan'))
        )
    );
};
...
const PCManagement = () => {
    ...
    useEffect(() => { const q = query(collection(db, 'pc_slots')); const unsubscribe = onSnapshot(q, (snapshot) => setPcSlots(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })))); return () => unsubscribe(); }, []);
    const handleStatusChange = (id, newStatus) => { if (newStatus === 'DIGUNAKAN') { setSelectedPcId(id); setInputModalOpen(true); } else { updateDoc(doc(db, 'pc_slots', id), { status: newStatus, currentUser: null, startTime: null, durationHours: null }); } };
    const handleUserInputSubmit = async (userName, durationHours) => { if (userName && durationHours > 0 && selectedPcId) { await updateDoc(doc(db, 'pc_slots', selectedPcId), { status: 'DIGUNAKAN', currentUser: userName, startTime: serverTimestamp(), durationHours: durationHours }); } setInputModalOpen(false); setSelectedPcId(null); };
...
export default function AdminPage() {
    ...
    useEffect(() => { if (!loading) { if (!user) { router.push('/login'); } else if (userData?.role !== 'admin') { router.push('/'); } } }, [user, userData, loading, router]);
    useEffect(() => { if (userData?.role !== 'admin') return; const q = query(collection(db, 'verification_requests'), where('status', '==', 'pending')); const unsubscribe = onSnapshot(q, (snapshot) => setVerifRequests(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })))); return () => unsubscribe(); }, [userData]);
    useEffect(() => { if (userData?.role !== 'admin' || !userData?.adminId) return; const q = query(collection(db, "transactions"), where("adminId", "==", userData.adminId)); const unsubscribe = onSnapshot(q, (snapshot) => setTransactions(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })))); return () => unsubscribe(); }, [userData]);
    useEffect(() => { if (userData?.role !== 'admin') return; const q = query(collection(db, 'game_accounts')); const unsubscribe = onSnapshot(q, (snapshot) => setGameAccounts(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })))); return () => unsubscribe(); }, [userData]);
    ...
    const handleOpenCompleteModal = (transaction) => { setCompletingTransaction(transaction); setCompleteModalOpen(true); };
    ...
    return React.createElement('div', { className: 'container mx-auto px-4 py-8' },
        ...
        React.createElement('div', { className: 'flex items-center mb-6' }, React.createElement(Shield, { className: 'h-8 w-8 text-discord-blurple mr-3' }), React.createElement('h1', { className: 'text-3xl font-extrabold' }, 'Admin Panel')),
        React.createElement('div', { className: 'flex border-b border-discord-darker mb-6 flex-wrap' }, React.createElement('button', { onClick: () => setActiveTab('transactions'), className: `py-2 px-4 font-semibold ${activeTab === 'transactions' ? 'text-white border-b-2 border-discord-blurple' : 'text-discord-gray'}` }, 'Manajemen Transaksi'), React.createElement('button', { onClick: () => setActiveTab('pcs'), className: `py-2 px-4 font-semibold ${activeTab === 'pcs' ? 'text-white border-b-2 border-discord-blurple' : 'text-discord-gray'}` }, 'Manajemen PC'), React.createElement('button', { onClick: () => setActiveTab('accounts'), className: `py-2 px-4 font-semibold ${activeTab === 'accounts' ? 'text-white border-b-2 border-discord-blurple' : 'text-discord-gray'}` }, 'Manajemen Akun'), React.createElement('button', { onClick: () => setActiveTab('verifications'), className: `py-2 px-4 font-semibold ${activeTab === 'verifications' ? 'text-white border-b-2 border-discord-blurple' : 'text-discord-gray'}` }, `Verifikasi User (${verifRequests.length})`)),
