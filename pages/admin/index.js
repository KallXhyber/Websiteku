...
import { Shield, UserCheck, XCircle, Clock, ListOrdered, Check, X, Gamepad2, PlusCircle, Edit, Trash2 } from 'lucide-react';
...
// --- KOMPONEN MODAL UNTUK DETAIL VERIFIKASI ---
const VerificationModal = ({ request, onClose }) => {
  ...
  return React.createElement(motion.div, {
    ...
    React.createElement(motion.div, {
        className: 'bg-discord-dark rounded-lg p-6 w-full max-w-2xl relative text-white',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const dataToSave = {
                ...
                adminName: userData.displayName || userData.email.split('@')[0],
                adminWa: userData.whatsapp || '',
                isSold: account?.isSold || false,
                updatedAt: serverTimestamp(),
            };
            
            if (account) {
                await updateDoc(doc(db, 'game_accounts', account.id), dataToSave);
            } else {
                await addDoc(collection(db, 'game_accounts'), { ...dataToSave, createdAt: serverTimestamp() });
            }
            onClose();
...
export default function AdminPage() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            setVerifRequests(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
        });
        return () => unsubscribe();
    }, [userData]);
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            setGameAccounts(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
        });
        return () => unsubscribe();
    }, [userData]);

    const handleTransactionStatus = async (id, newStatus) => {
    ...
    if (loading || !userData || userData.role !== 'admin') {
        return React.createElement('div', { className: 'flex justify-center items-center h-screen' }, 'Memverifikasi akses admin...');
    }
    ...
    return React.createElement('div', { className: 'container mx-auto px-4 py-8' },
        ...
        React.createElement('div', { className: 'flex border-b border-discord-darker mb-6' },
        ...
        React.createElement(AnimatePresence, { mode: 'wait' },
            activeTab === 'transactions' ? React.createElement(motion.div, { key: 'transactions', initial: { opacity: 0 }, animate: { opacity: 1 }, exit: { opacity: 0 } },
                    ...
                    React.createElement('div', { className: 'bg-black/20 border border-discord-darker p-4 rounded-lg space-y-3' },
                        pendingTransactions.length > 0 ? pendingTransactions.map(tx => React.createElement('div', { key: tx.id, className: 'bg-discord-darker p-3 rounded-lg flex justify-between items-center' },
                            React.createElement('div', null,
                                React.createElement('p', { className: 'font-bold' }, tx.userEmail),
                                React.createElement('p', { className: 'text-sm text-discord-gray' }, `Billing: ${tx.billingId}`)
                            ),
                            React.createElement('div', { className: 'flex gap-2' },
                                React.createElement('button', { onClick: () => handleTransactionStatus(tx.id, 'dibatalkan'), className: 'bg-red-600 p-2 rounded hover:bg-red-700 text-white' }, React.createElement(X, {size:16})),
                                React.createElement('button', { onClick: () => handleTransactionStatus(tx.id, 'aktif'), className: 'bg-green-600 p-2 rounded hover:bg-green-700 text-white' }, React.createElement(Check, {size:16}))
                            )
                    ...
                    React.createElement('h2', { className: 'text-2xl font-bold mb-4 flex items-center' }, React.createElement(ListOrdered, { size: 20, className: 'mr-2' }), 'Transaksi Aktif'),
                     React.createElement('div', { className: 'bg-black/20 border border-discord-darker p-4 rounded-lg space-y-3' },
                        activeTransactions.length > 0 ? activeTransactions.map(tx => React.createElement('div', { key: tx.id, className: 'bg-discord-darker p-3 rounded-lg flex justify-between items-center' },
                            ...
                            React.createElement('button', { onClick: () => handleTransactionStatus(tx.id, 'selesai'), className: 'bg-discord-blurple py-2 px-3 rounded font-semibold text-sm' }, 'Selesaikan')
                        )) : React.createElement('p', { className: 'text-discord-gray text-center p-4' }, 'Tidak ada transaksi aktif.')
                    )
                )
            ) :
            activeTab === 'accounts' ? React.createElement(motion.div, { key: 'accounts', initial: { opacity: 0 }, animate: { opacity: 1 }, exit: { opacity: 0 } },
                React.createElement('div', { className: 'flex justify-between items-center mb-4' },
                    ...
                    React.createElement('button', { onClick: () => { setEditingAccount(null); setAccountModalOpen(true); }, className: 'bg-green-600 text-white py-2 px-4 rounded flex items-center hover:bg-green-700' }, React.createElement(PlusCircle, { size: 18, className: 'mr-2' }), 'Tambah Produk')
                ),
                React.createElement('div', { className: 'bg-black/20 border border-discord-darker p-4 rounded-lg space-y-3' },
                    gameAccounts.length > 0 ? gameAccounts.map(acc => React.createElement('div', { key: acc.id, className: 'bg-discord-darker p-3 rounded-lg flex justify-between items-center' },
                        React.createElement('div', null,
                            React.createElement('p', { className: 'font-bold' }, acc.gameName),
                            React.createElement('p', { className: 'text-sm text-discord-gray' }, `Rp ${acc.price.toLocaleString('id-ID')}`)
                        ),
                        React.createElement('div', { className: 'flex gap-2' },
                            React.createElement('button', { onClick: () => { setEditingAccount(acc); setAccountModalOpen(true); }, className: 'bg-blue-600 p-2 rounded hover:bg-blue-700 text-white' }, React.createElement(Edit, { size: 16 })),
                            React.createElement('button', { onClick: () => handleDeleteAccount(acc.id), className: 'bg-red-600 p-2 rounded hover:bg-red-700 text-white' }, React.createElement(Trash2, { size: 16 }))
                        )
                    )) : React.createElement('p', { className: 'text-discord-gray text-center p-4' }, 'Belum ada produk akun game.')
                )
            ) :
            React.createElement(motion.div, { key: 'verifications', initial: { opacity: 0 }, animate: { opacity: 1 }, exit: { opacity: 0 } },
                React.createElement('div', { className: 'bg-black/20 border border-discord-darker p-6 rounded-lg' },
                    verifRequests.length > 0 ? React.createElement('div', { className: 'space-y-4' },
                        verifRequests.map(req => React.createElement('div', {
                            key: req.id,
                            className: 'bg-discord-darker p-4 rounded-lg flex justify-between items-center'
                        },
                            React.createElement('div', null,
                                React.createElement('p', { className: 'font-bold' }, req.userEmail),
                                React.createElement('p', { className: 'text-sm text-discord-gray flex items-center' },
                                    React.createElement(Clock, {size:14, className:'mr-2'}),
                                    `Dikirim: ${req.submittedAt ? new Date(req.submittedAt.toDate()).toLocaleTimeString() : 'N/A'}`
                                )
                            ),
                            React.createElement('button', {
                                onClick: () => setSelectedRequest(req),
                                className: 'bg-discord-blurple py-2 px-4 rounded font-semibold hover:bg-opacity-80'
                            }, 'Lihat Detail')
                        ))
