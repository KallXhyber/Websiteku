...
import { Server, ShieldCheck, MessageSquare, ExternalLink, Plus, Minus } from 'lucide-react';
...
// --- DATA ADMIN BARU DENGAN STRUKTUR HARGA PAKET ---
const adminData = [
  { 
    ...
    pricing: [
        ...
        { id: 'siang', name: 'Paket Siang (06:00 - 18:00)', price: 20000, type: 'fixed' },
        { id: 'malam', name: 'Paket Malam (18:00 - 06:00)', price: 25000, type: 'fixed' }
    ]
  },
  { 
...
const VerificationPopup = ({ onClose }) => {
    ...
    const handleVerificationSubmit = async (e) => {
        e.preventDefault();
        if (!user) {
            setFeedback({ type: 'error', message: 'Anda harus login untuk verifikasi.' });
            return;
        }
        if (!files.discord || !files.steam || !files.cfx) {
            setFeedback({ type: 'error', message: 'Harap upload ketiga screenshot.' });
            return;
        }
        setIsLoading(true);
        ...
        try {
            ...
            await setDoc(doc(db, 'verification_requests', user.uid), {
                ...
                discordUrl: fileUrls.discord, steamUrl: fileUrls.steam, cfxUrl: fileUrls.cfx,
                message: message, status: 'pending', submittedAt: serverTimestamp(),
            });
            await setDoc(doc(db, 'users', user.uid), { verificationStatus: 'pending' }, { merge: true });
            setFeedback({ type: 'success', message: 'Verifikasi berhasil dikirim! Mohon tunggu konfirmasi admin.' });
            setTimeout(onClose, 3000);
        } catch (error) {
            console.error("Error verifikasi:", error);
            setFeedback({ type: 'error', message: 'Gagal mengupload file. Silakan coba lagi.' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return React.createElement(motion.div, { className: 'fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4' },
        React.createElement(motion.div, { className: 'bg-discord-dark rounded-lg p-8 w-full max-w-lg relative' },
            React.createElement('button', { onClick: onClose, className: 'absolute top-4 right-4 text-gray-400 hover:text-white' }, 'X'),
            React.createElement('div', { className: 'text-center' },
                React.createElement(ShieldCheck, { className: 'mx-auto h-12 w-12 text-discord-blurple mb-4' }),
                React.createElement('h2', { className: 'text-2xl font-bold' }, 'Verifikasi Diri')
            ),
            React.createElement('form', { onSubmit: handleVerificationSubmit, className: 'space-y-4 mt-6' },
                React.createElement(FileInput, { label: 'Screenshot Akun Discord', type: 'discord', file: files.discord }),
                React.createElement(FileInput, { label: 'Screenshot Akun Steam', type: 'steam', file: files.steam }),
                React.createElement(FileInput, { label: 'Screenshot Akun Cfx.re', type: 'cfx', file: files.cfx }),
                React.createElement('textarea', { value: message, onChange: (e) => setMessage(e.target.value), placeholder: 'Pesan tambahan... (opsional)', rows: 2, className: 'w-full bg-discord-darker text-white p-2 rounded-lg' }),
...
const TransactionPopup = ({ admin, user, paket, totalHarga, jumlahJam, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (billingId.length < 4) {
            setError('Harap isi minimal 4 digit terakhir nomor WA Anda.');
            return;
        }
        setIsLoading(true);
        setError('');
    ...
    return React.createElement(motion.div, { className: 'fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4' },
        React.createElement(motion.div, { className: 'bg-discord-dark rounded-lg p-8 w-full max-w-md relative' },
            React.createElement('button', { onClick: onClose, className: 'absolute top-4 right-4 text-gray-400 hover:text-white' }, 'X'),
            React.createElement('div', { className: 'text-center' },
                React.createElement(MessageSquare, { className: 'mx-auto h-12 w-12 text-discord-blurple mb-4' }),
                React.createElement('h2', { className: 'text-2xl font-bold' }, 'Konfirmasi Sewa'),
                React.createElement('p', { className: 'text-discord-gray mt-2' }, `Anda akan menyewa dari admin ${admin.name}.`)
            ),
            React.createElement('form', { onSubmit: handleSubmit, className: 'space-y-4 mt-6' },
                React.createElement('div', {},
                    React.createElement('label', { htmlFor: 'billingId', className: 'block text-sm font-medium text-discord-gray mb-1' }, '4 Digit Terakhir No. WA (untuk Billing)'),
                ...
                React.createElement('button', { type: 'submit', disabled: isLoading, className: 'w-full bg-green-600 text-white font-bold py-3 rounded-full flex items-center justify-center' },
                    isLoading ? 'Memproses...' : 'Lanjut ke WhatsApp',
                    !isLoading && React.createElement(ExternalLink, {size: 18, className: 'ml-2'})
                )
            )
        )
    );
...
export default function SewaPage() {
  ...
  useEffect(() => {
    ...
    if (selectedPackage.type === 'hourly') {
      setTotalPrice(selectedPackage.price * hours);
    } else {
  ...
  const handleSewa = () => {
    ...
    if (userData?.verificationStatus === 'terverifikasi') {
      setShowTransactionPopup(true);
    } else if (userData?.verificationStatus === 'pending') {
      alert('Verifikasi Anda sedang ditinjau oleh Admin. Mohon tunggu konfirmasi.');
    } else {
  ...
  return React.createElement('div', { className: 'container mx-auto px-4 py-8' },
    ...
    showTransactionPopup && selectedAdmin && user && React.createElement(TransactionPopup, { 
        admin: selectedAdmin, 
        user: user,
    ...
    React.createElement(motion.div, {
      ...
      React.createElement('div', { className: 'text-center mb-10' },
        React.createElement(Server, { className: 'mx-auto h-12 w-12 text-discord-blurple mb-4' }),
        React.createElement('h1', { className: 'text-4xl font-extrabold' }, 'Sewa Cloud PC'),
        React.createElement('p', { className: 'text-discord-gray mt-2' }, 'Satu spesifikasi untuk semua kebutuhan gaming Anda.')
      ),
      React.createElement('div', { className: 'mb-8' },
        React.createElement('h2', { className: 'text-2xl font-bold mb-4 text-center' }, '1. Pilih Admin yang Bertugas'),
        React.createElement('div', { className: 'grid grid-cols-1 md:grid-cols-3 gap-4' },
          admins.map(admin => React.createElement('button', {
              key: admin.id,
              onClick: () => handleAdminSelect(admin),
              className: `p-4 rounded-lg border-2 text-left transition-all ${selectedAdmin?.id === admin.id ? 'border-discord-blurple bg-discord-blurple/20' : 'border-discord-darker'} ${admin.isOnline ? 'cursor-pointer hover:border-discord-blurple' : 'opacity-50 cursor-not-allowed'}`
            },
              React.createElement('div', { className: 'flex justify-between items-center mb-2' },
                React.createElement('h3', { className: 'text-lg font-bold' }, admin.name),
                React.createElement('div', { className: `px-2 py-1 text-xs rounded-full ${admin.isOnline ? 'bg-green-500 text-white' : 'bg-gray-600 text-gray-300'}` }, admin.isOnline ? 'Online' : 'Offline')
              ),
      ...
      selectedAdmin && React.createElement(motion.div, {
        ...
        React.createElement('div', { className: 'grid grid-cols-2 md:grid-cols-4 gap-4' },
            selectedAdmin.pricing.map(pkg => React.createElement('button', {
                key: pkg.id,
                onClick: () => handlePackageSelect(pkg),
                className: `p-3 rounded-lg border-2 text-center transition-all ${selectedPackage?.id === pkg.id ? 'border-discord-blurple bg-discord-blurple/20' : 'border-discord-darker hover:border-discord-blurple'}`
            },
